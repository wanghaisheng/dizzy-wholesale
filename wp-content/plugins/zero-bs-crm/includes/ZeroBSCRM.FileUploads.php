...
/* ======================================================
   ...
   function zeroBSCRM_privatisedDirCheck($echo=false){
		...
		#} JetpackRebrandRelook = this is public facing on the System page. Will editing this break anything?
   		$privateDirName = 'zbscrm-store';

   		if (isset($wpUploadsDir) && is_array($wpUploadsDir)){

   			if (isset($wpUploadsDir['basedir']) && !empty($wpUploadsDir['basedir'])){


   				$finalDirPath = $wpUploadsDir['basedir'].'/'.$privateDirName;
   				$finalURL = $wpUploadsDir['baseurl'].'/'.$privateDirName;

   				// check existence
   				if (!is_dir($finalDirPath)){

   					// doesn't exist, attempt to create
   					mkdir($finalDirPath, 0755, true);
   					// force perms?
   					chmod($finalDirPath, 0755);

   					if (is_dir($finalDirPath)) return array('path'=>$finalDirPath,'url'=>$finalURL);

   				} else { 

   					// exists

   						// check perms? (later if req.)

   					return array('path'=>$finalDirPath,'url'=>$finalURL);
   ...
   function zeroBSCRM_privatisedDirCheckWorks($echo=false){
   		...
   		$wpUploadsDir = wp_upload_dir();
   		$privateDirName = 'zbscrm-store/_wip';
