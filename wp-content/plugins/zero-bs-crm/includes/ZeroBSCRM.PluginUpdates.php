...
class zeroBSCRM_Plugin_Updater {
	...
	public function check_update($transient){
		...
		/* 
			...
			// this acts as a blocker, effectively stopping weird twice-fired issue
			// actually can't do this as it's the SECOND FIRE which is actually adding?
			//if (!defined('ZBS_UPDATES_JUST_CHECKED')){
			// this was another attempt to stop it 'regetting'
			... but I think this is basically firing for every plugin
			... so for now v1.0 licensing, we'll just cache the api_request (in that func api_request)
			... and let this reget each time (saves multi-calls)

			if ($zbsUpdateCheckCount < 5){
			global $zbsUpdateCheckCount; 
			if (!isset($zbsUpdateCheckCount)) 
				$zbsUpdateCheckCount = 1;
			else
				$zbsUpdateCheckCount++;

				// define blocker to stop double fire
				define('ZBS_UPDATES_JUST_CHECKED',time());

			//debugecho 'run: '.$zbsUpdateCheckCount.'<br>transient:<pre>'; print_r($transient); echo '</pre><hr />';
		...
		*/
		...
		if($installedExts > 0 || $this->get_license_key() !== false){
			...
			$response = $this->api_request( 'all_info', array('slug'=> 'all', 'zbs-extensions'=> $zbs_extensions_on_site, 'active-extensions' => $active_plugins, 'telemetry-active' => $zbs_active_plugins, 'telemetry-all' => $zbs_all_plugins_and_ver)); //, 'license'=>$lk
		  	...
		  	if ( ! is_wp_error( $response )) {
				...
				if (isset($response['license_key_valid']) && ($response['license_key_valid'] == 'false' || $response['license_key_valid'] == 'empty' || $response['license_key_valid'] == '' || $response['license_key_valid'] == 'expired')){
					...
					if (isset($response['devmode'])){
						...
						// invalid license key
						$key_info['validity'] = ''; if (isset($key_info['validity'])) $key_info['validity'] = $response['license_key_valid'];
						...
						if(!get_transient( 'zbs-nag-license-key-now' )){
							...
							$parameters = __("Your License Key is incorrect ","zero-bs-crm"). "<a href='". $link ."'>" . __("Please enter your license key</a>.", "zero-bs-crm");
							$reference = time();
				...
				// so next check if it's a rebranded one
					...
					$key_info['validity'] = ''; if (isset($key_info['validity'])) $key_info['validity'] = 'brand.template';
					...
					if(!get_transient( 'zbs-nag-license-key-now' )){
						...
						// generate nag
						$link = admin_url('admin.php?page=zerobscrm-plugin-settings&tab=license');
						$parameters = "Your License Key is not linked to a CRM Brand (Error #401) ". "<a href='". $link ."'>" . __("Please enter your license key</a>.", "zero-bs-crm");
						$reference = time();
						$cid = get_current_user_id();
				...
				// otherwise this is a legit key :)
					...
					if (isset($response['access'])) $key_info['access'] = sanitize_text_field($response['access']);
					if (isset($response['expires'])) $key_info['expires'] = (int)sanitize_text_field($response['expires']);
					...
					if (isset($response['claimed'])){
						...
						global $zbsLicenseClaimed; $zbsLicenseClaimed = true;
				...
				foreach ($zbs_extensions_on_site as $name => $pluginDetails){
					...
					if (is_array($official_ver)) foreach ($official_ver as $extUpdateInfo){
						...
						if (isset($extUpdateInfo['zbskey']) && $extUpdateInfo['zbskey'] == $pluginDetails['key']){
							...
							if (version_compare( $pluginDetails['ver'], $extUpdateInfo['version'], '<' )){
								...
								$e = false; if (is_array($pluginDetails) && isset($pluginDetails['path'])) $e = explode('/',$pluginDetails['path']);
								if (is_array($e)) $newSlug = $e[0];
								if (empty($newSlug)) $newSlug = $extUpdateInfo['slug'];
								...
								$obj->package 									= $modifiedDLLink;
									...
									if (isset($extUpdateInfo['url'])) $obj->url = $extUpdateInfo['url'];
								...
								$parameters = __("Please update ", "zero-bs-crm") . $pluginDetails['name'] . __(" from version ", "zero-bs-crm") . $pluginDetails['ver'] . __(" to version ", "zero-bs-crm") .  $extUpdateInfo['version'];
				...
				if (zeroBSCRM_isWL()){
					...
					if (isset($core) && is_array($core) && isset($core['ver'])){
						...
						$coreUpdateInfo = false; if (is_array($official_ver)) foreach ($official_ver as $extUpdateInfo){ if (isset($extUpdateInfo['zbskey']) && $extUpdateInfo['zbskey'] == 'core') { $coreUpdateInfo = $extUpdateInfo; break; } }
						...
						if ($coreUpdateInfo && version_compare( $core['ver'], $coreUpdateInfo['version'], '<' )){
							...
							$e = explode('/',$core['path']);
							if (is_array($e)) $newSlug = $e[0];
							if (empty($newSlug)) $newSlug = $coreUpdateInfo['slug'];
							...
							// generate nag
							// then we are adding the notification to "please update Jetpack CRM extension "X")
							$parameters = __("Please update ", "zero-bs-crm") . $core['name'] . __(" from version ", "zero-bs-crm") . $core['ver'] . __(" to version ", "zero-bs-crm") .  $coreUpdateInfo['version'];
		...
		/* PERFORMANCE CHECKS  - STILL SEEMS TO FIRE TWICE WHEN LOADING /wp-admin/update-core.php
			$zbs_check_update_end = time();
			$the_time = $zbs_check_update_end - $zbs_check_update_start;
			if($updateCheck){
				//zbs_write_log("checking performance time taken " . $the_time);
			} 
		*/
...
function zeroBSCRM_isLocal($fullCheck=true) {
	...
	// quick caching
	if (defined('ZBS_DEVOVERRIDE')) return false;
