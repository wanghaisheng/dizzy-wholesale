		...
		if ($autoLogThis > 0){
			...
			if ($zbs->isDAL3()){
				...
				if ( is_array($quote) && count($quote['contact']) > 0 ){
					...
					if (!empty($signedBy)) $noteLongDesc = __('Signed','zero-bs-crm').' '.$signedBy;
	...
	function zeroBSCRM_IA_NewEventLog($obj=array()){
		...
		if ($autoLogThis > 0 && isset($zbsNoteAgainstPostID) && !empty($zbsNoteAgainstPostID)){
			...
			if ($zbs->isDAL3()){
				...
				if (!empty($taskDescription)) $noteLongDesc = $taskDescription;
				if (!empty($taskStart)) {
					...
					// add starting date
					$noteLongDesc .= __('Starts at ','zero-bs-crm').' '.$taskStart;
