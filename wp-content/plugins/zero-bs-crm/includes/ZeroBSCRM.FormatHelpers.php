...
/* ======================================================
  / Table Views (temporary)
   ...
   function zeroBSCRM_format_migrationVersion($ver=''){
		...
		if (substr($migrationName,0,1) == '4') {
		$migrationName = '4.'.substr($migrationName,1);
		}
		...
		if (
			...
			// catch x.0.x
			if (strlen($migrationName) == 4 && substr($migrationName,2,1) == '0') $migrationName = str_replace('0','.0.',$migrationName);
			$migrationName = str_replace('..','.',$migrationName);
