...
#} CLONED FROM getCustomers 27/07/16
#} ignores $withQuotes var
function zeroBS_getCustomersNoQJ($withFullDetails=false,$perPage=10,$page=0,$withInvoices=false,$withQuotes=false,$searchPhrase='',$withTransactions=false,$argsOverride=false){
	...
	#$zbsQPI['retrieveCustomers2getCustomers'] = zeroBSCRM_mtime_float();
		...
		foreach ($customerList as $customerEle){
			...
			#} WITH QUOTES ALWAYS SELECTED HERE, as NEED THEM TO FILTER :/
			$withQuotes = true;
		                			...
		                			if (!isset($fieldDeets['migrate']) || $fieldDeets['migrate'] != 'addresses'){
		                				...
		                				if (!isset($master['meta'][$fieldKey]) || empty($master['meta'][$fieldKey])){
		                					...
		                					if (isset($slave['meta'][$fieldKey]) && !empty($slave['meta'][$fieldKey])){
			                					...
			                					$conflictingChanges[] = __('Field not copied',"zero-bs-crm").' "'.$fieldDeets[1].'" '.__('from secondary record over main record, (main had value). Value was',"zero-bs-crm").' "'.$slave['meta'][$fieldKey].'"';
