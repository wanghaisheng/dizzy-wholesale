...
function zeroBS_getCustomersNoQJ($withFullDetails=false,$perPage=10,$page=0,$withInvoices=false,$withQuotes=false,$searchPhrase='',$withTransactions=false,$argsOverride=false){
	...
	// fullDetails effectively ALWAYS on
	// also $argsOverride will not do anything, (so errored out here so we can spot)

	if ($argsOverride !== false){

		throw new Exception('LEGACY USE OF getContacts detected:zeroBS_getCustomersNoQJ');
		exit();

	}
	global $zbs; return $zbs->DAL->contacts->getContacts(array(

			'searchPhrase' => $searchPhrase, // searches which fields?
...
function zeroBSCRM_createClientPortalUser($cID=-1,$email='',$passwordLength=12,$fname='',$lname=''){
	...
	if (!empty($cID) && !empty($email) && zeroBSCRM_validateEmail($email)){
		...
		if ( null == email_exists( $email )) {
			...
			if ( ! empty( $fname ) ) {
				$wpUserDeets['first_name'] = $fname;
			}
			if ( ! empty($lname ) ) { 
				$wpUserDeets['last_name'] = $lname;
			}
			...
			if ( $user->exists() ) {
				...
				// add role(s)
				if ( ! empty( $extraRole ) ) {
					$user->set_role( $extraRole );	// Set the WP role first, then the JPCRM role
...
function zeroBS_getCustomerExternalSource($cID=-1){
	...
	zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v2.4+. Please use zeroBS_getExternalSource()');
...
function zeroBSCRM_getCPTLogs($customerID=false,$withFullDetails=false,$perPage=100,$page=0,$searchPhrase='',$argsOverride=false){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. zeroBSCRM_getCPTLogs should no longer be used (no logs stored in CPT).');
		return array();
...
/* ======================================================
   ...
   function zeroBS___________GenericHelpers(){return;}
	...
	function zeroBS_deleteGeneric($id=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. (zeroBS_deleteGeneric)');
...
/* ======================================================
   ...
   function zeroBS___________CompanyHelpers(){return;}
	...
	function zeroBS_getCompanyMeta($coID=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. Please use zeroBS_getCompanyMeta()');
	...
	function zeroBS_getCompanyExternalSource($cID=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. Please use zeroBS_getExternalSource()');
...
/* ======================================================
   ...
   // if returnAsInt - will return -1 for not published, -2 for not accepted, or 14int timestamp for accepted
	...
	function zeroBS_getQuoteByZBSID($zbsQuoteID=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. (zeroBS_getQuoteByZBSID) Please use $zbs->DAL->quotes->getQuote()');
	...
	function zeroBS_getQuotePostIDFromZBSID($zbsQuoteID=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. (zeroBS_getQuotePostIDFromZBSID) Please use $zbs->DAL->quotes->getQuote()');
...
/* ======================================================
   ...
   function zeroBS___________InvoiceHelpers(){return;}
	...
	function zeroBS_getInvoicePostIDFromZBSID($zbsInvID=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. (zeroBS_getInvoicePostIDFromZBSID) Please use $zbs->DAL->invoices->getInvoice()');
...
/* ======================================================
   ...
   function zeroBS_addUpdateEvent($eventID = -1, $eventFields = array(), $reminders=array()){
	...
	function zeroBSCRM_task_getMeta($ID = -1){
			...
			zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. zeroBSCRM_task_getMeta should now be replaced with proper zbs->DAL->calls');
	...
	function zeroBSCRM_task_updateMeta($ID = -1, $meta = array()){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. zeroBSCRM_task_updateMeta should now be replaced with proper zbs->DAL->calls');
...
/* ======================================================
   ...
   ====================================================== */
	...
	function zeroBSCRM_clearCPTAutoDrafts(){
				...
				if (isset($zbsCustomPostTypes) && is_array($zbsCustomPostTypes)) foreach ($zbsCustomPostTypes as $cpt){
	...
	function zeroBSCRM_invoice_canView($cID=-1, $invID=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. zeroBSCRM_invoice_canView should now be replaced with zeroBSCRM_invoice_getContactAssigned (or DAL call better)');
	...
	function zeroBSCRM_quote_canView($cID=-1, $quoteID=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. zeroBSCRM_quote_canView should now be replaced with zeroBSCRM_quote_getContactAssigned (or DAL call better)');
	...
	*/
	function zbsLink($key='',$id=-1,$type='zerobs_customer',$prefixOnly=false,$taxonomy=false){
		...
		$objTypeID = -1;
		// already an objTypeID? 
		if ($zbs->DAL->isValidObjTypeID($type))
			$objTypeID = (int)$type;
		else
			// translate type into typeStr (contact/zerobs_customer => 1)
			$objTypeID = $zbs->DAL->objTypeID($type);
...
/* ======================================================
    ...
    */
	function zeroBS_getCustomerExtrasViaSQL($customerID=-1){
		...
		zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. zeroBS_getCustomerExtrasViaSQL should no longer be used.');
	...
	function zeroBS_checkOwner($objID=-1,$potentialOwnerID=-1,$allowNoOwnerAccess=true,$objType=false){
		...
		if (!$objType) 
			zeroBSCRM_DEPRECATEDMSG('ZBS Function Deprecated in v3.0+. zeroBS_checkOwner should no longer be used, ESPECIALLY without 4th param (user type). Use zeroBSCRM_checkObjectOwner');
		else {
			global $zbs;
