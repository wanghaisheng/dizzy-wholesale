   ...
   ====================================================== */
    ...
    class zeroBS__Metabox_Transaction extends zeroBS__Metabox{ 
        ...
        public function html( $transaction, $metabox ) {
		    	...
		    	if ($contactID > 0) $contactName = zeroBS_customerName($contactID,false,true,false); 
					...
					switch ($prefill_obj){
						...
						case ZBS_TYPE_CONTACT:
							...
							if ($prefill_id > 0){
								...
								$contactName = zeroBS_customerName($contactID,false,true,false); 
		                    	...
		                    	if (zeroBSCRM_getSetting('companylevelcustomers') != "1"){ 
		                    		...
		                    		?><div class="ui grid"><div class="seven wide column">
			                    		...
			                    		<div id="invoiceFieldWrap" style="position:relative;display:none;max-width:658px" class="assignInvToCust"><input style="max-width:200px" id="invoice_id" name="invoice_id" value="<?php if(isset($transaction['invoice_id'])){ echo $transaction['invoice_id']; } ?>" class="form-control" autocomplete="zbstra-<?php echo time(); ?>-<?php echo rand(0,100); ?>" /></div><?php
			                    		 ...
			                    		 ?></div><div class="two wide column centered"><?php _e('Or','zero-bs-crm'); ?></div><div class="seven wide column"><div id="zbs-company-title"><label for"company-ta"><?php echo jpcrm_label_company(); ?></label></div><?php
        ...
        public function save_data( $transactionID, $transaction ) {
            ...
            if (!defined('ZBS_T_SAVED')){
                ...
                if (empty($transactionID) || $transactionID < 1)  $transactionID = -1;
					...
					// if not passed in the UI set them to the transaction date
					$usePaidDates = zeroBSCRM_getSetting('paiddatestransaction');
					if ($usePaidDates != 1){
						$transaction['date_completed'] 	= $transaction['date'];
						$transaction['date_paid']		= $transaction['date'];
					}
					...
					$transaction['currency'] = zeroBSCRM_getCurrencyStr();
			            ...
			            if ($zbsTransactionContact > 0) $transaction['contacts'] = array($zbsTransactionContact);
			            $zbsTransactionCompany = (int)sanitize_text_field($_POST['zbsct_company']);
			            if ($zbsTransactionCompany > 0) $transaction['companies'] = array($zbsTransactionCompany);
            ...
            $transactionID = -1; if (is_array($transaction) && isset($transaction['id'])) $transactionID = (int)$transaction['id'];
                ...
                if ($transactionID > 0){ // existing
                    ...
                    <div class="zbs-transaction-actions-bottom zbs-objedit-actions-bottom">
                        ...
                        ?><div id="zbs-transaction-actions-delete zbs-objedit-actions-delete">
