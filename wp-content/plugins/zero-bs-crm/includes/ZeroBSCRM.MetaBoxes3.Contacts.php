    ...
    class zeroBS__Metabox_Contact extends zeroBS__Metabox{ 
        ...
        public function save_data( $contact_id, $contact ) {
            ...
            if (!defined('ZBS_C_SAVED')){
                ...
                // This is adapted from our previous, spread-out save for contact fields :) no need for autosave/nonce checks as done by parent class
                // ... and as is our simpler metaboxes, we can avoid all this save_post checking :)

                /* Old method (DB1) 
                if ($zbs->DBVER < 2){

                    // retrieve from POST + update
                    update_post_meta($contact_id, 'zbs_customer_meta', zeroBS_buildContactMeta($_POST));
                    
                    #} Save any company (parent)
                    if (isset($_POST['zbs_company'])) zbsCRM_addUpdateCustomerCompany($contact_id,(int)sanitize_text_field($_POST['zbs_company'])); #update_post_meta($post_id,'zbs_company',(int)sanitize_text_field($_POST['zbs_company']));

                    #AVATARSAVE - save any avatar change if changed :)
                    #} hacky side-ways load until DB2 (after which is all native non wp)
                    $avatarURL = ''; if (isset($_POST['zbs-contact-avatar-custom-url']) && !empty($_POST['zbs-contact-avatar-custom-url'])) $avatarURL = sanitize_text_field( $_POST['zbs-contact-avatar-custom-url'] );
                    if (!empty($avatarURL)) zeroBSCRM_Generate_Featured_Image($avatarURL,$contact_id);

                    // Update Msg
                    // not like this, use global pageMessages add_action( 'zerobs_updatemsg_contact', array( $this, 'updateMessage' ), 1, 2 );
                    $this->updateMessage();
                    ...
                    */
                    ...
                    /* this is now dealt with in buildMeta 
                    // have to translate old fields
                    $dataArr['secaddr1'] = $dataArr['secaddr_addr1'];
                    $dataArr['secaddr2'] = $dataArr['secaddr_addr2'];
                    $dataArr['seccity'] = $dataArr['secaddr_city'];
                    $dataArr['seccounty'] = $dataArr['secaddr_county'];
                    $dataArr['seccountry'] = $dataArr['secaddr_country'];
                    $dataArr['secpostcode'] = $dataArr['secaddr_postcode'];
                    */
                    //... by using buildContactMeta, custom fields are 'dealt with' automatically
                ...
                /* Don't need any of these anymore, all moved into DAL2 Proper
                #} Is new customer? (passed from metabox html)
                #} Internal Automator
                if (isset($_POST['zbscrm_newcustomer']) && $_POST['zbscrm_newcustomer'] == 1){

                    /* This shouldn't be needed as is dealt with by addUpdate 
                    ...
                    * /
                    ...
                    if (isset($_POST['zbsc_status_dirtyflag']) && $_POST['zbsc_status_dirtyflag'] == "1"){
                        ...
                        // from?
                        // to
                        $status = ''; if (isset($contactData['status'])) $status = $contactData['status'];

                        // IA
                        zeroBSCRM_FireInternalAutomator('customer.status.update',array(
                            'id'=>$contact_id, 
                            'againstid' => $contact_id,
                            'userMeta'=> $contactData,
                            'to'=>$status
                            ));
...
class zeroBS__Metabox_ContactActions extends zeroBS__Metabox{
    ...
    public function html( $contact, $metabox ) {
        ...
        $avatarMode = zeroBSCRM_getSetting('avatarmode'); $avatarStr = '';
        if ($avatarMode !== "3") {
        ...
        if (!empty($avatarStr)){
            ...
            if ($avatarMode == 1) $segmentClass = 'ui segment';
        ...
        jQuery(document).ready(function(){
            ...
            // action items
            jQuery('.zbs-contact-action').off('click').click(function(){
                    ...
                    ?>
                            <table class="form-table wh-metatab wptbp zbsFileSlotTable" data-sloturl="<?php echo $fileSlotURL; ?>" id="<?php echo $this->metaboxID; ?>-tab">
                                ...
                                <?php #adapted from http://code.tutsplus.com/articles/attaching-files-to-your-posts-using-wordpress-custom-meta-boxes-part-1--wp-22291
                                        ...
                                        $html .= '<input type="file" id="zbsc_file_'.$filePerma.'" name="zbsc_file_'.$filePerma.'" value="" size="25" class="zbs-dc">';
...
class zeroBS__Metabox_ContactPortal extends zeroBS__Metabox{
    ...
    public function __construct( $plugin_file ) {
        ...
        $this->metaboxTitle = __('Contact Portal',"zero-bs-crm");
        $this->metaboxScreen = 'zbs-add-edit-contact-edit'; //'zerobs_edit_contact'; // we can use anything here as is now using our func
        $this->metaboxArea = 'side';
        $this->metaboxLocation = 'high';
        $this->capabilities = array(
    ...
    public function html( $contact, $metabox ) {
        ...
        if($wp_user_id != -1 && $wp_user_id > 0){
            ...
            echo '<div class="zbs-customerportal-activeuser">';
                ...
                echo '<span class="ui label">'.$userObj->user_login.'</span>';
            ...
            echo '<hr /><div class="zbs-customerportal-activeuser-actions">';
                ...
                _e('Customer Portal Access:',"zero-bs-crm");
                ...
                if ($customerPortalActive){
                    ...
                    echo '<div id="zbs-customerportal-access-actions">';
                        ...
                        if ($zbs->isDAL2()) echo '<button type="button" id="zbs-customerportal-resetpw" class="ui mini button orange">'.__('Reset Password',"zero-bs-crm").'</button>';
            ...
            _e('Save your contact, or add an email to enable Customer Portal functionality',"zero-bs-crm");
        }
    ...
    public function html( $contact, $metabox ) {
        ...
        if (gettype($zbsCustomer) != "array"){
            ...
            ?><div id="zbs-aka-alias-input-wrap">
                <input type="text" class="zbs-aka-alias-input" placeholder="<?php _e('Add Alias.. e.g.','jetpackcrm.com'); ?> mike2@domain.com" />
            ...
            jQuery(document).ready(function(){
                ...
                jQuery('.zbs-aka-alias-input').keydown(function(){
                ...
                jQuery('#zbs-aka-alias-add').off('click').click(function(){
            ...
            function zeroBSJS_bindAKAMode(){
                ...
                // hover over
                jQuery('.zbs-aka-alias').hover(function () {
                    jQuery(this).toggleClass("hovering");        
                });

                // remoe aka
                jQuery('.zbs-aka-alias-remove').off('click').click(function(){
