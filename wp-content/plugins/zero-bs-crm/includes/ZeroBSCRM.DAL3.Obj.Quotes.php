		...
		if ($update){
			...
			*/
				...
				if ($wpdb->update( 
						...
						array( // where data types
							...
							if (!isset($limitedFields) || !is_array($limitedFields) || $limitedFields == -1){
								...
								// tag work?
								if (isset($data['tags']) && is_array($data['tags'])) $this->addUpdateQuoteTags(array('id'=>$id,'tagIDs'=>$data['tags'],'mode'=>'replace'));
			...
			if ($wpdb->insert( 
					...
					// tag work?
					if (isset($data['tags']) && is_array($data['tags'])) $this->addUpdateQuoteTags(array('id'=>$newID,'tagIDs'=>$data['tags'],'mode'=>'replace'));
