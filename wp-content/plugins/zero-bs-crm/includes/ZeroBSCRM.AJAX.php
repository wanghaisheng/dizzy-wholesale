...
/* ======================================================
   ...
   function jpcrm_hide_woo_promo(){
	$option = update_option( 'jpcrm_hide_woo_promo', 'hide' );
	wp_die();
   }

   #} Dash date range picker keeping as AJAX longeer term be good to have this as WP REST API and React led.
  ...
  // Table names $ZBSCRM_t['contacts'] and $ZBSCRM_t['transactions']
	...
	*/
	...
	$from = sanitize_text_field($_POST['s']);
	$to = sanitize_text_field($_POST['e']);

	$from = strtotime($from);
	$to = date_create( $to )->setTime( 23, 59, 59 )->getTimestamp();

	//first up, we want to get contacts between the two dates, grouped by status
	$sql 			 = $wpdb->prepare("SELECT count(ID) as count, zbsc_status FROM " . $ZBSCRM_t['contacts'] . " WHERE zbsc_created > %d AND zbsc_created < %d GROUP BY zbsc_status", $from, $to);
	$contact_summary = $wpdb->get_results($sql);

	$total_boxes = 1;
	$contact_total = 0;
	$summary[0] = $contact_total;
	foreach($contact_summary as $k => $v){
		if($total_boxes > 14){
			//semantic can only handle 16 boxes. Do people use more than that as statuses? plus we add in transactions and contact total
			//maybe we can let people pick which statuses to display here. Prior to this it was very inflexible and only showed specific
			//statuses and was not language friendly.
		}
		$summary[] = array(
						'total' => $v->count,
						'label' => $v->zbsc_status
					);
		$contact_total = $contact_total + $v->count;
		$summary[0] = array(
						'total' => $contact_total,
						'label' => __('Contacts')
		)			;
	...
	$sql 			 	= $wpdb->prepare("SELECT count(ID) FROM " . $ZBSCRM_t['transactions'] . " WHERE zbst_date > %d AND zbst_date < %d", $from, $to);
	$transaction_total 	= $wpdb->get_var($sql);
	$summary[] = array(
									'total' => $transaction_total,
									'label' => __('Transactions', 'jetpackcrm')
								);
	$total_boxes++;

	//semantic needs the words not the numeral for the class. stored this in the FormatHelpers.php from SO.
	$total_string = jetpackcrm_convertNumberToWord($total_boxes);

	//next we want the contact chart which is total contacts between the dates grouped by day, week, month, year
	$sql 			= $wpdb->prepare("SELECT count(ID) as count, YEAR(FROM_UNIXTIME(zbsc_created)) as year FROM " . $ZBSCRM_t['contacts'] . " WHERE zbsc_created > %d AND zbsc_created < %d GROUP BY year ORDER BY year", $from, $to);
	$yearly 		= $wpdb->get_results($sql);

	$sql 			= $wpdb->prepare("SELECT count(ID) as count, zbsc_created as ts, MONTH(FROM_UNIXTIME(zbsc_created)) as month, YEAR(FROM_UNIXTIME(zbsc_created)) as year FROM " . $ZBSCRM_t['contacts'] . " WHERE zbsc_created > %d AND zbsc_created < %d GROUP BY month, year ORDER BY year, month", $from, $to);
	$monthly 		= $wpdb->get_results($sql);

	$sql 			= $wpdb->prepare("SELECT count(ID) as count, zbsc_created as ts, WEEK(FROM_UNIXTIME(zbsc_created)) as week, YEAR(FROM_UNIXTIME(zbsc_created)) as year FROM " . $ZBSCRM_t['contacts'] . " WHERE zbsc_created > %d AND zbsc_created < %d GROUP BY week, year ORDER BY year, week", $from, $to);
	$weekly			= $wpdb->get_results($sql);

	$sql 			= $wpdb->prepare("SELECT count(ID) as count, zbsc_created as ts, DAY(FROM_UNIXTIME(zbsc_created)) as day, MONTH(FROM_UNIXTIME(zbsc_created)) as month, YEAR(FROM_UNIXTIME(zbsc_created)) as year FROM " . $ZBSCRM_t['contacts'] . " WHERE zbsc_created > %d AND zbsc_created < %d GROUP BY day, month, year ORDER BY year, month, day", $from, $to);
	$daily			= $wpdb->get_results($sql);


	$zeros = jetpackcrm_create_zeros_array($from, $to);
   ...
   # apply action to portal user (enable disable)
	...
	function zbs_wizard_fin(){
	  check_ajax_referer( 'zbswf-ajax-nonce', 'security' );  //nonce to bounce out if not from right page 
	  if ( current_user_can( 'manage_options' ) ) {   //only admin can do this too (extra security layer)

	  	$runCount = get_option('zbs_wizard_run',0);

	  	//we use ZBS on line 386 below but do not define it in this function until later down
	  	global $zbs; 

			/* adjusted 21/02/17 - now sends to slack inc how many times run 

	  	//the finito step of the setup wizard. Also use this to ping our slack channel the setup settings
	  	if(get_option('zbs_wizard_run') == 0){
	  		//wizard hasn't run so set the options and send the information to our slack channel
	        ...
	        #not used yet $crm_b2b = sanitize_text_field($_POST['zbs_b2b']);
			#now set by menu_style $crm_override = (int)sanitize_text_field($_POST['zbs_crm_override']);
			$crm_share = 0; if (isset($_POST['zbs_crm_share_essentials']) && $_POST['zbs_crm_share_essentials'] == "1") $crm_share = 1;
			...
			$initOptions = array( 'share'=> $crm_share, 'bn' => $bn, 'fn' => $fn , 'ln' => $ln, 'em' => $em, 'emv' => $emv, 'smm' => time(), 'n' => $crm_name, 'u' => $crm_url, 'o' => $crm_other, 's' => $crm_sub, 't' => $crm_type, 'ov' => $crm_override, 'eq' => $crm_enable_quotes, 'ei' => $crm_enable_invoices, 'ef' => $crm_enable_forms, 'ems' => $crm_menu_style, 'v' => $zbs->version, 'cu' => $crm_curr);
			...
			if (is_callable('curl_init') && ($crm_share == 1 || $crm_sub == 1)){   //only share if permission given
			    ...
			    $postData = http_build_query($m);
			  	...
			  	$ch = curl_init();
			  	curl_setopt($ch, CURLOPT_URL,  $zbs->urls['smm'] );
			    ...
			    $response = wp_remote_post( $zbs->urls['smm'], array(
				    'method'      => 'POST',
				    'timeout'     => 45,
				    'redirection' => 5,
				    'httpversion' => '1.0',
			...
			#} Setting changes #} Need this - delcared above (11 11 2018)
			//global $zbs;
				...
				switch ($crm_menu_style){
					...
					case 1: 
						...
						#} full (normal) wp
						#$zbs->settings->update( 'wptakeovermodeforall' , 0);
				...
				if ($crm_enable_invoices == 1) {
					...
					#zeroBSCRM_extension_uninstall_pdfinv();
			...
			if (is_array($currentTaxTables) && count($currentTaxTables) == 0 && is_array($currSetting) && isset($currSetting['strval'])){
				...
				// add any
				if (count($ratesToAdd) > 0) foreach ($ratesToAdd as $rate){

					 zeroBSCRM_taxRates_addUpdateTaxRate(array(
			...
			#} Keep count of reruns

		//	zbs_write_log("this should be installing and counting here...");
	...
	*/
	...
	function zeroBSCRM_AJAX_addLog(){
		...
		if (isset($_POST['zbsnshortdesc']) && !empty($_POST['zbsnshortdesc'])) $zbsNoteShortDesc = zeroBSCRM_preDBStr(sanitize_text_field($_POST['zbsnshortdesc']));
		if (isset($_POST['zbsnlongdesc']) && !empty($_POST['zbsnlongdesc'])) $zbsNoteLongDesc = zeroBSCRM_preDBStr(zeroBSCRM_textProcess(zeroBSCRM_stripExceptLineBreaks(nl2br($_POST['zbsnlongdesc']))));

		#} Meta vals, extras
		
		$zbsNoteObjType = ''; if (isset($_POST['zbsnobjtype']) && !empty($_POST['zbsnobjtype'])) $zbsNoteObjType = zeroBSCRM_textProcess($_POST['zbsnobjtype']);
		...
		if (
			...
			$res = zeroBS_addUpdateLog($zbsNoteAgainstPostID,$zbsNoteIDtoUpdate,-1,array(
	...
	function zeroBSCRM_AJAX_updateLog(){
	    ...
	    // if (!zeroBSCRM_permsCustomers()) exit('{processed:-1}');
	    if (!zeroBSCRM_permsLogsAddEdit()) exit('{processed:-1}');
		...
		if (
			...
			// DAL2+ needs to know obj type :)zbsNoteObjType - passed as last var
			...
			#if (!empty($newOrUpdatedLogID)) $res = 1;
...
function ZeroBSCRM_get_quote_template(){
    ...
    if ($customer_ID !== -1 && $quote_template_ID !== -1){
	    ...
	    if (isset($_POST['quote_fields']) && $zbs->isDAL3() && is_array($_POST['quote_fields'])){
	    	...
	    	if (isset($_POST['quote_fields']['zbscq_title']) && !empty($_POST['quote_fields']['zbscq_date'])) $quote_date = sanitize_text_field( $_POST['quote_fields']['zbscq_date'] );
	...
	function zeroBSCRM_AJAX_mailDelivery_validateSMTPPorts(){
		...
		if (!empty($smtpPort)){
			...
			if (!$remotePortCheck[0]){
				$res['debugs'][] = __('Jetpack CRM cannot connect to '.$smtpHost.' on port: '.$smtpPort.', this may not matter, as ZBS will try other ports for you below.','zero-bs-crm'); 
				$okay = false;
			} 		
					...
					if (is_array($attemptedSend) && isset($attemptedSend['sent']) && $attemptedSend['sent']){
						...
						if (isset($attemptedSend['finset']) && is_array($attemptedSend['finset'])){
								...
								if ( ! function_exists( 'zeroBSCRM_encrypt' ) ) {
									require( ZEROBSCRM_PATH . 'includes/ZeroBSCRM.Encryption.php' );
								}
...
====================================================== */
	...
	function zeroBSCRM_AJAX_updateListViewFilterButtons(){
		...
		#} Check perms
		if (!current_user_can('administrator')) { header('Content-Type: application/json'); exit('{err:1}'); }

			global $zbs;

			#} Retrieve type + columns arr
			$listtype = sanitize_text_field($_POST['listtype']);
			$listButtons = $_POST['v']; // NEEDS SANITATION!
	        ...
	        */
	        ...
	        if (in_array($listtype,$acceptableListViewTypes)){
	        	...
	        	#} Build
	        	$newButtons = array(); foreach ($listButtons as $buttonKey => $buttonVal){

	        		$newButtons[$buttonVal['fieldstr']] = array(__($buttonVal['namestr'],"zero-bs-crm"));
	        		$passBack[] = array('fieldstr' => $buttonVal['fieldstr'], 'namestr' => __($buttonVal['namestr'],"zero-bs-crm"));

	        	}

	        	#} Update	    			
	        	$newCustomViews[$listtype.'_filters'] = $newButtons;
				$zbs->settings->update( 'customviews2' , $newCustomViews);
...
/* RATHER than a global switch, just do via $var ^^
			...
			switch ($listtype){
		        ...
		        #} Brutal save over anyway..
		        	...
		        	#} Build
		        	$newCustomerButtons = array(); foreach ($listButtons as $buttonKey => $buttonVal){

		        		$newCustomerButtons[$buttonVal['fieldstr']] = array(__($buttonVal['namestr'],"zero-bs-crm"));
		        		$passBack[] = array('fieldstr' => $buttonVal['fieldstr'], 'namestr' => __($buttonVal['namestr'],"zero-bs-crm"));

		        	}

		        	#} Update	    			
		        	$newCustomViews['customer_filters'] = $newCustomerButtons;
					$zbs->settings->update( 'customviews2' , $newCustomViews);
		        ...
		        #} Brutal save over anyway..
		        	...
		        	#} Build
		        	$newCompanyButtons = array(); foreach ($listButtons as $buttonKey => $buttonVal){

		        		$newCompanyButtons[$buttonVal['fieldstr']] = array(__($buttonVal['namestr'],"zero-bs-crm"));
		        		$passBack[] = array('fieldstr' => $buttonVal['fieldstr'], 'namestr' => __($buttonVal['namestr'],"zero-bs-crm"));

		        	}

		        	#} Update	    			
		        	$newCustomViews['company_filters'] = $newCompanyButtons;
					$zbs->settings->update( 'customviews2' , $newCustomViews);
		        ...
		        #} Brutal save over anyway..
		        	...
		        	#} Update	    			
		        	$newCustomViews['transaction_filters'] = $newCustomerButtons;
					$zbs->settings->update( 'customviews2' , $newCustomViews);
			...
			/* ==============================================================================
			 ===================== TRANSACTION =========================================== */
				...
				case 'transaction':
					...
					#} Retrieve data
					//$withFullDetails=false,$perPage=10,$page=0,$withCustomerDeets=false, $searchPhrase,$hasTagIDs=array(),$inArray=array(),$quickFilters=array()
					$transactions = zeroBS_getTransactions(true,$per_page,$page_number,$withCustomer,$possibleSearchTerm,$possibleTagIDs,$inArray,$sortField,$sortOrder,$withTags,$possibleQuickFilters);
		...
		switch ($listtype){
			...
			case 'customer':
				...
				if ($id > 0 && !empty($field)){
					...
					// translate field to db table name:
					$dbField = '';
					switch ($field){

						case 'status': $dbField = 'zbsc_status'; break;
						case 'assigned': $dbField = 'zbs_owner'; break;

					}

					if (!empty($dbField)){

						// has field and id, let's try brutal set.
						// rely on '%s' type - dangerous... but okay for now (status + assigned to is okay)
						$zbs->DAL->contacts->addUpdateContact(array(
							'id' => $id,
							'limitedFields' => array(array('key'=>$dbField,'val'=>$v,'type'=>'%s'))
							)); 
...
====================================================== */
	...
	function zeroBSCRM_AJAX_betaFeedback(){
		...
		global $zbs;
			...
			$emailHTML = zeroBSCRM_mailTemplates_directMsg(true,$content,$title);
