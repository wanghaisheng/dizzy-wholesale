...
function zeroBS_getCustomersNoQJ($withFullDetails=false,$perPage=10,$page=0,$withInvoices=false,$withQuotes=false,$searchPhrase='',$withTransactions=false,$argsOverride=false){
	...
	global $zbs; return $zbs->DAL->getContacts(array(
...
/* PRE MVP
function zeroBS_getCustomersNoQJ(){
		...
		foreach ($customerList as $customerEle){
			...
			if (count($retObj['quotes']) == 0){
				...
				#REWORKED # and status isn't Refused Quote
				#} now only if status IS lead or Customer
				if (isset($retObj['meta']) && isset($retObj['meta']['status']) && ($retObj['meta']['status'] == 'Lead' || $retObj['meta']['status'] == 'Customer')){
				#$retObj['meta']['status'] != 'Refused' && $retObj['meta']['status'] != 'Blacklisted'){
...
function zeroBS_getInvoicesv2($withFullDetails=false,$perPage=10,$page=0,$withCustomerDeets=false, $searchPhrase,$hasTagIDs=array(),$inArray=array(),$quickFilters=array()){
		...
		* /
		...
		foreach ($list as $ele){
			...
			$retObj['title'] = __("Invoice " . $ele->ID);
			$retObj['meta'] = get_post_meta($ele->ID,'zbs_customer_invoice_meta',true);
