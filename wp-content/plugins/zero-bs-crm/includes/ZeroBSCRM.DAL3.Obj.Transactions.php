...
class zbsDAL_transactions extends zbsDAL_ObjectLayer {
    ...
    protected $objectModel = array(
        ...
        private $transToIncludeInTotals = false; // got/updated via $this->getTransactionStatusesToInclude()
     ...
     */
     ...
     public function getTransactionTotalByMonth( $args=array() ){
        ...
        $sql = $wpdb->prepare("SELECT SUM(" . $column_prefix . "total) as total, MONTH(FROM_UNIXTIME(" . $column_prefix . "date)) as month, YEAR(FROM_UNIXTIME(" . $column_prefix . "date)) as year FROM " . $ZBSCRM_t['transactions'] . " WHERE " . $column_prefix . "date_paid > %d AND " . $column_prefix . "date_paid < %d".$transStatusQueryAdd." GROUP BY month, year ORDER BY year, month", $paidAfter, $paidBefore);
    ...
    public function addUpdateTransaction($args=array()){
        ...
        #} ========= CHECK force_uniques & not_empty & max_len ========
            ...
            if (!is_array($limitedFields)){
                ...
                // verify uniques
                if (!$this->verfiyUniqueValues($data,$id)) return false; // / fails unique field verify

                // verify not_empty
                if (!$this->verfiyNonEmptyValues($data)) return false; // / fails empty field verify
        ...
        if ($update){
                ...
                if ($wpdb->update( 
                        ...
                        array( // where data types
                            ...
                            if (!isset($limitedFields) || !is_array($limitedFields) || $limitedFields == -1){
                                ...
                                // tag work?
                                if (isset($data['tags']) && is_array($data['tags'])) $this->addUpdateTransactionTags(array('id'=>$id,'tagIDs'=>$data['tags'],'mode'=>'replace'));
            ...
            if ($wpdb->insert( 
                    ...
                    // tag work?
                    if (isset($data['tags']) && is_array($data['tags'])) $this->addUpdateTransactionTags(array('id'=>$newID,'tagIDs'=>$data['tags'],'mode'=>'replace'));
    ...
    private function tidy_transaction($obj=false,$withCustomFields=false){
            ...
            */
            ...
            $res['title'] = $this->stripSlashes($obj->zbst_title);
     ...
     */
    public function transactionAccountingType($transactionType='Sale'){

        $default = 'debit';

        if (!empty($transactionStatus)){

                if ($transactionType == __('Refund','zero-bs-crm')) return 'credit';
                if ($transactionType == __('Credit Note','zero-bs-crm')) return 'credit';
    ...
    function getTransactionStatusesToInclude(){
        ...
        // cache
        if (isset($this->transToIncludeInTotals) && $this->transToIncludeInTotals !== false && !empty($this->transToIncludeInTotals)) return $this->transToIncludeInTotals;

        // load
        $setting = $this->DAL()->setting('transinclude_status','all');
