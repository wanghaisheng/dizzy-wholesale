...
/* ======================================================
  WordPress Button/Text Overides (could end up in language inc?)
   ====================================================== */
	...
	#} Change 'Publish' to 'Save': http://wordpress.stackexchange.com/questions/3578/change-the-text-on-the-publish-button
   ...
   ====================================================== */
	...
	function zeroBSCRM_footer_admin ($text) 
	{
		...
		if (zeroBSCRM_isAdminPage()){
			echo '<span id="footer-thankyou">'.__('Thank you for using',"zero-bs-crm").' <a href="'.$zbs->urls['home'].'" target="_blank">'.__('Jetpack CRM','zero-bs-crm').'</a></span>.';
			return '';
		}
	...
	function zeroBSCRM_footer_ver($content='') 
	{
		...
		if (zeroBSCRM_isAdminPage()){
				...
				if ($showThanks) return __('Jetpack CRM',"zero-bs-crm").' '.__('Version',"zero-bs-crm").' '.$zbs->version;
				return '';
		}
