...
function zeroBSCRM_mailDelivery_accountDDL($selectedOption=-1,$id='zbs-mail-delivery-acc',$showProtocol=true,$withSemanticWrap=true){
	...
	if ($withSemanticWrap){
		...
		?><script type="text/javascript">jQuery(document).ready(function(){ jQuery('#<?php echo $id; ?>').dropdown();});</script><?php
...
function zeroBSCRM_mailDelivery_defaultFromname(){
    ...
    #} biz name
    $r =  zeroBSCRM_getSetting('businessname');

	#} crm name:
	if (empty($r)) $r = zeroBSCRM_getSetting('customheadertext');
		
	#} Blog name:
	if (empty($r)) $r = get_bloginfo('name');
...
function zeroBSCRM_mailDelivery_sendMessage($mailDeliveryAccKey='',$mail=-1){
	...
	if (isset($mailSettings) && is_array($mailSettings)){
		...
		if (!isset($mailSettings) || !is_array($mailSettings) || !isset($mailSettings['fromemail'])){
			...
			return array(false,'errorMsg'=>__('<span>#289</span>Could not load settings for this mail delivery option'));
		...
		if (!isset($mail) || !is_array($mail) || !isset($mail['toEmail'])){
			...
			return array(false,'errorMsg'=>__('<span>#290</span>Could not load message for this mail delivery call'));
	...
	EXAMPLE: 10/05/18 wh
	    ...
	    */
		...
		$sendToEmail = ''; $subject = ''; $headers = array(); $body = ''; $textBody = '';
		$options = array(
			'html' => 1
		);
		...
		// do final checks & send per mode
			...
			if (empty($body) || empty($sendToEmail) || !zeroBSCRM_validateEmail($sendToEmail)) {
				...
				return array(false,'errorMsg'=>__('<span>#291</span>Could not process message details for this mail delivery call'));
		...
		switch ($mailSettings['mode']){
			...
			case 'wp_mail':
				...
				// WP mail needs to check from is in header (if someone's added, override :))
					...
					*/
					...
					// sends to itself to test
					$sent = wp_mail( $sendToEmail, $subject, $body, $headers, $attachments );
			...
			case 'smtp':
						...
						$sent = zeroBSCRM_mailDelivery_sendViaSMTP(
								...
								#} SMTP Settings
								$mailSettings['host'],$mailSettings['port'],$mailSettings['user'],zeroBSCRM_mailDelivery_retKeyData($mailSettings['pass']),
			...
			default:
				...
				return array(false,'errorMsg'=>__('<span>#289</span>Mail Delivery mode not found'));
				break;
	...
	return array(false,'errorMsg'=>__('<span>#297</span>No settings found for this mail delivery option'));
