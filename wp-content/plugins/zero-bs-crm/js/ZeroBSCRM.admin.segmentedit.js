...
function zeroBSCRMJS_segment_bindPostRender(){
	...
	jQuery('.zbs-segment-edit-var-condition-type').off('change').change(function(){
	...
	// on change of type, rebuild line
	jQuery('.zbs-segment-edit-var-condition-operator').off('change').change(function(){
	...
	jQuery('#zbs-segment-edit-act-add-condition').off('click').click(function(){
	...
	// remove conditions
	jQuery('.zbs-segment-edit-condition-remove').off('click').click(function(){
	...
	// hover over remove
	jQuery('.zbs-segment-edit-condition-remove').off('hover').hover(
       function(){ jQuery(this).addClass('orange') },
       function(){ jQuery(this).removeClass('orange') }
	);
	

	// preview audience / continue
	jQuery('#zbs-segment-edit-act-p2preview').off('click').click(function(){
	...
	// save segment
	jQuery('#zbs-segment-edit-act-p2submit, #zbs-segment-edit-act-save').off('click').click(function(){
	...
	jQuery('#zbs-segment-edit-act-back').off('click').click(function(){
...
function zeroBSCRMJS_segment_buildConditions(){
	...
	// this builds/rebuilds the conditions html based on zbsSegment obj
...
function zeroBSCRMJS_segment_buildConditionLine(rule){
	...
	//console.log('building line',rule);
...
function zeroBSCRMJS_segment_buildConditionCascadesForEle(ele){
	...
	if (typeof selected != "undefined"){
		...
		if (typeof window.zbsAvailableConditions[selected] != "undefined" && typeof window.zbsAvailableConditions[selected].operators != "undefined" && window.zbsAvailableConditions[selected].operators.length > 0 && window.zbsAvailableConditions[selected].operators[0] != "tag"){
			...
			// is tagged? pass hidden input
			if (window.zbsAvailableConditions[selected].operators[0] == "tag") html += '<input type="hidden" class="zbs-segment-edit-var-condition-operator" value="tag" />';
		...
		if (typeof selected != "undefined"){
			...
			switch (selected){
				...
				case 'daterange':
					...
					html += '<input type="text" class="zbs-date-range-condition zbs-segment-edit-var-condition-value" value="'+v+'" />';
				...
				case 'tag':
					...
					html += '<select class="zbs-segment-edit-var-condition-value ui dropdown">';
						...
						if (typeof window.zbsAvailableTags != "undefined" && window.zbsAvailableTags.length > 0){

							jQuery.each(window.zbsAvailableTags,function(ind2,ele2){
	...
	if (errors == 0){
		...
		// continue
			...
			if (sconditions.length > 0){
				...
				// debug
				//console.log('post',[sname,smatchtype,sconditions]);
				...
				zeroBSCRMJS_segment_previewSegment(segment,function(previewList){
					...
					//console.log('RETRIEVED PREVIEW: ',previewList);
						// localise
						var seg = segment;
						...
						jQuery('#zbs-segment-edit-act-save, #zbs-segment-edit-act-delete').removeClass('loading').removeAttr('disabled');
			...
			if (sconditions.length > 0){
				...
				zeroBSCRMJS_segment_saveSegment(segment,function(id){
					...
					//console.log('ret',id);
						...
						jQuery('#zbs-segment-edit-act-p2submit').removeClass('loading').removeAttr('disabled');
						jQuery('#zbs-segment-edit-act-p2preview').removeClass('loading').removeAttr('disabled');
						jQuery('#zbs-segment-edit-act-save, #zbs-segment-edit-act-delete').removeClass('loading').removeAttr('disabled');
