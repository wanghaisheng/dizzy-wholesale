...
function zbscrmJS_retrieveCurrentBillToEmail(){
	...
	if (typeof potentialEmail == "undefined"){				
...
function zbscrm_JS_draw_send_invoice_to(res){
	...
	// here we allow for prefilled data via zbsprefillcust _GET param (passed by php)
	var billTo = res.invoiceObj.bill;
	if ((billTo == null || billTo == '') && typeof window.zbsJS_prefillemail != "undefined" && typeof window.zbsJS_prefillid != "undefined"){
		...
		html += '</div>';
			...
			//html += '<div class="inv-to-form">' + zbscrm_JS_invoice_lang('send_to') +':</div>';
                html += '<div class="wh-large inv-to-input">';
						...
						html += '<input class="form-control typeahead" type="email" id="zbs_inv_bill" name="zbscq_bill" value="' + billTo + '" placeholder="' + zbscrm_JS_invoice_lang('bill_to') + '" />';
						html += '</div>';
...
function zbscrm_JS_draw_partials_table(res){
	...
	var html = '<div id="zbs-invoice-partial-payments" class = "ui grid">';
		...
		jQuery.each(res.invoiceObj.partials, function (i, v) {
			...
			html += '<div class="zbs-partial-value half ri">' + parseFloat(vValue).toFixed(zbs_root.currencyOptions.noOfDecimals) + '</div>';
			html += '</div>';
	...
	jQuery('#billing-to .typeahead').typeahead({
		...
		templates: {
			suggestion: function (v) {
				ico = '<i class="user icon"></i>';
				if (v.obj_type == 2){
					ico = '<i class="building icon"></i>';
				}
				var name = zbscrm_JS_invoice_lang('noname'); if (typeof v.name != "undefined") name = v.name;
...
function zbscrm_JS_bind_invoice_actions(){
	...
	// send email button
	jQuery('#zbs_invoicing_send_email').off('click').click(function(ind,ele){
	      ...
	      //allowOutsideClick: false
	        ...
	        if (result.value){
				...
				if (typeof invEmail != "undefined" && invEmail != '' && zbscrm_JS_validateEmail(invEmail) && window.invoice_id > 0){
		            ...
		            // get settings
		            var attachassoc = -1; if (jQuery('#zbs_invoice_email_modal_attachassoc').length > 0 && jQuery('#zbs_invoice_email_modal_attachassoc').is(':checked')) attachassoc = 1;
		            var attachpdf = -1; if (jQuery('#zbs_invoice_email_modal_attachaspdf').length > 0 && jQuery('#zbs_invoice_email_modal_attachaspdf').is(':checked')) attachpdf = 1;
		            var params = {
