...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function filter_primary_nav_menu_dropdown_symbol( $title, $item, $args, $depth ) {
		...
		if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
			$title = '<span class="nav-drop-title-wrap">' . $title . '<span class="dropdown-nav-toggle" aria-pressed="false" aria-label="' . esc_attr__( 'Expand child menu', 'kadence' ) . '">' . kadence()->get_icon( 'arrow-down' ) . '</span></span>';
		}
	...
	public function filter_mobile_nav_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
		...
		if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
			...
			$output = '<div class="drawer-nav-drop-wrap">' . $item_output . '<button class="drawer-sub-toggle" data-toggle-duration="10" data-toggle-target="' . esc_attr( $toggle_target_string ) . '" aria-expanded="false"><span class="screen-reader-text">' . esc_html__( 'Expand child menu', 'kadence' ) . '</span>' . kadence()->get_icon( 'arrow-down', '', false, false ) . '</button></div>';
			return $output;
		}
